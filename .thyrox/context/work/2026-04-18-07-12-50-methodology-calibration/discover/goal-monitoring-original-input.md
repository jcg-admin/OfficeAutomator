```yml
created_at: 2026-04-19 10:24:48
project: THYROX
...
status: Borrador
version: 1.0.0
fuente: "Chapter 11: Goal Setting and Monitoring — VERSIÓN ORIGINAL EPUB en inglés" (documento externo, 2026-04-19)
nota: |
  Versión original del EPUB en inglés, extraída mediante script Python (AsciiDoc).
  La versión española (goal-monitoring-pattern-input.md) fue analizada previamente:
  63.3% PARCIALMENTE CALIBRADO.
  Esta versión original tiene contenido adicional no presente en la traducción:
  - Sección "Expert Code Review Approach" (nueva)
  - Sección "Visual Summary" con imagen fig2
  - Ejemplos 2 y 3 comentados en el código
  - Conclusión más extensa con "accountable AI systems"
  - Función to_snake_case preservada como dead code (igual que traducción)
  - Caveats más extendidos con "Expert Code Review Approach"
  Diferencia técnica clave: response == "true" (inglés) vs "verdadero" (español)
  — mismo bug potencial si LLM responde con mayúscula ("True" vs "true").
```

# Input: Chapter 11 — Goal Setting and Monitoring (original EPUB, AsciiDoc, preservado verbatim)
...
Source: EPUB extraction via AsciiDoc
...
1. **Customer Support Automation** — An agent's goal might be to "resolve customer's billing inquiry." It monitors the conversation, checks database entries, and uses tools to adjust billing. Success is monitored by confirming the billing change and receiving positive customer feedback. If the issue isn't resolved, it escalates.

2. **Personalized Learning Systems** — A learning agent might have the goal to "improve students' understanding of algebra." It monitors the student's progress on exercises, adapts teaching materials, and tracks performance metrics like accuracy and completion time, adjusting its approach if the student struggles.

3. **Project Management Assistants** — An agent could be tasked with "ensuring project milestone X is completed by Y date." It monitors task statuses, team communications, and resource availability, flagging delays and suggesting corrective actions if the goal is at risk.

4. **Automated Trading Bots** — A trading agent's goal might be to "maximize portfolio gains while staying within risk tolerance." It continuously monitors market data, its current portfolio value, and risk indicators, executing trades when conditions align with its goals and adjusting strategy if risk thresholds are breached.

5. **Robotics and Autonomous Vehicles** — An autonomous vehicle's primary goal is "safely transport passengers from A to B." It constantly monitors its environment (other vehicles, pedestrians, traffic signals), its own state (speed, fuel), and its progress along the planned route, adapting its driving behavior to achieve the goal safely and efficiently.

6. **Content Moderation** — An agent's goal could be to "identify and remove harmful content from platform X." It monitors incoming content, applies classification models, and tracks metrics like false positives/negatives, adjusting its filtering criteria or escalating ambiguous cases to human reviewers.
...
To illustrate the Goal Setting and Monitoring pattern, we have an example using LangChain and OpenAI APIs. This Python script outlines an autonomous AI agent engineered to generate and refine Python code.

Its core function is to produce solutions for specified problems, ensuring adherence to user-defined quality benchmarks.
...
You can best understand this script by imagining it as an autonomous AI programmer assigned to a project. The process begins when you hand the AI a detailed project brief, which is the specific coding problem it needs to solve.
...
- Uses an LLM (like GPT-4o) to generate and refine Python code until the 
  goals are met. (I am using max 5 iterations, this could be based on a set goal as well)
...
def generate_prompt(
    ...
    base_prompt = f"""
You are an AI coding agent. Your job is to write Python code based on the following:
...
Please critique this code and identify if the goals are met. Mention if improvements are needed
for clarity, simplicity, correctness, edge case handling, or test coverage.
...
**Image reference in original:** `image::images/image1.jpg[Goal Setting and Monitor example]`
...
### Expert Code Review Approach (section present in original, absent in translation)
...
When reviewing a code snippet, you should:
...
### A More Robust Multi-Agent Approach

A more robust approach involves separating these concerns by giving specific roles to a crew of agents. For instance, one can build a personal crew of AI agents where each has a specific role:
...
**Visual Summary:** `image::images/image2.png[Goal design patterns]`
...
**Nota 1 — SyntaxError en el script de extracción:**
El script Python que generó este AsciiDoc falló con `SyntaxError: invalid decimal literal` en línea 181.
Causa: `adoc_content = """..."""` contiene el docstring del código Python ejemplo (también `"""`),
lo que termina el string externo prematuramente. Solución: usar `'''...'''` para `adoc_content`
o escapar los triple-quotes internos como `\"""`.
El contenido del AsciiDoc fue preservado desde el mensaje del usuario — no desde el archivo generado.

**Nota 2 — `response == "true"` (inglés) vs. `"verdadero"` (traducción):**
El original inglés usa `response == "true"`. Si el LLM responde "True" (capital T),
la comparación falla silenciosamente — `goals_met` retorna False aunque el LLM dijo True.
El `.strip().lower()` previene el problema de capitalización: `"True".lower() == "true"` ✓.
Sin embargo, si el LLM responde "True." o "True!\n", la comparación falla.
Este es el mismo bug que la traducción española (`.strip().lower()` está presente en ambas).

**Nota 3 — `to_snake_case` es dead code (igual que traducción):**
Definida pero nunca llamada. `save_code_to_file` implementa su propia variante inline.

**Nota 4 — Terminación silenciosa igual que traducción:**
El loop no tiene `for...else` — si `goals_met` nunca retorna True, el código guarda
el último intento sin advertencia. Presente en ambas versiones.

**Nota 5 — Secciones presentes en original y ausentes en traducción:**
- "Expert Code Review Approach" — presente en original, ausente en traducción española
- Visual Summary con `image2.png` — presente en original, ausente en traducción
- Ejemplos 2 y 3 comentados — presentes en original, ausentes en traducción
- Conclusión con "accountable AI systems" — más extensa en original

**Nota 6 — Claim ADK de Google en Key Takeaways:**
"In Google's ADK, goals are often conveyed through agent instructions, with monitoring
accomplished through state management and tool interactions."
El código del capítulo usa LangChain + OpenAI, no Google ADK. Sin URL de documentación.
