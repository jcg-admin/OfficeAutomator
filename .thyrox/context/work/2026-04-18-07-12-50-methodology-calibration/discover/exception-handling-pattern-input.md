```yml
created_at: 2026-04-19 10:28:54
project: THYROX
...
nota: |
  ...
  Diferencia notable respecto a capítulos anteriores: 3 referencias reales
  (McConnell 2004, Shi et al. arXiv:2412.00534, O'Neill 2022 Electronics).
  Código usa Google ADK (SequentialAgent) — diferente de Cap.11 (LangChain).
  Herramientas get_precise_location_info y get_general_area_info no están definidas.
```

# Input: Chapter 12 — Exception Handling and Recovery (texto completo, preservado verbatim)

---

## INTRODUCCIÓN

For AI agents to operate reliably in diverse real-world environments, they must be able to manage unforeseen situations, errors, and malfunctions.

Just as humans adapt to unexpected obstacles, intelligent agents need robust systems to detect problems, initiate recovery procedures, or at least ensure controlled failure.

This essential requirement forms the basis of the Exception Handling and Recovery pattern.
...
It emphasizes the importance of both proactive preparation and reactive strategies to ensure continuous operation, even when facing challenges.
...
The capacity to handle unexpected events ensures these AI systems are not only intelligent but also stable and reliable, which fosters greater confidence in their deployment and performance.

Integrating comprehensive monitoring and diagnostic tools further strengthens an agent's ability to quickly identify and address issues, preventing potential disruptions and ensuring smoother operation in evolving conditions.
...
This pattern may sometimes be used with reflection.

For example, if an initial attempt fails and raises an exception, a reflective process can analyze the failure and reattempt the task with a refined approach, such as an improved prompt, to resolve the error.

---

## Exception Handling and Recovery Pattern Overview

The Exception Handling and Recovery pattern addresses the need for AI agents to manage operational failures.

This pattern involves anticipating potential issues, such as tool errors or service unavailability, and developing strategies to mitigate them.

These strategies may include error logging, retries, fallbacks, graceful degradation, and notifications.
...
Implementing this pattern enhances the reliability and robustness of AI agents, allowing them to function in unpredictable environments.

Examples of practical applications include chatbots managing database errors, trading bots handling financial errors, and smart home agents addressing device malfunctions.

The pattern ensures that agents can continue to operate effectively despite encountering complexities and failures.

**[image: Fig.1 — Key components of exception handling and recovery for AI agents]**

### Error Detection

This involves meticulously identifying operational issues as they arise.
...
Additionally, monitoring by other agents or specialized monitoring systems might be implemented for more proactive anomaly detection, enabling the system to catch potential issues before they escalate.

### Error Handling

Once an error is detected, a carefully thought-out response plan is essential.
...
Where complete recovery is not immediately possible, the agent can maintain partial functionality to provide at least some value (graceful degradation).

Finally, alerting human operators or other agents might be crucial for situations that require human intervention or collaboration (notification).
...
Implementation of this robust exception handling and recovery pattern can transform AI agents from fragile and unreliable systems into robust, dependable components capable of operating effectively and resiliently in challenging and highly unpredictable environments.
...
Consider, for instance, an agent's response to a failed tool call.

Such failures can stem from incorrect tool input or issues with an external service that the tool depends on.
...
**[image: Fig.2 — Exception handling pattern]**
...
This chapter explores the Exception Handling and Recovery pattern, which is essential for developing robust and dependable AI agents.

This pattern addresses how AI agents can identify and manage unexpected issues, implement appropriate responses, and recover to a stable operational state.

The chapter discusses various aspects of this pattern, including the detection of errors, the handling of these errors through mechanisms such as logging, retries, and fallbacks, and the strategies used to restore the agent or system to proper function.

Practical applications of the Exception Handling and Recovery pattern are illustrated across several domains to demonstrate its relevance in handling real-world complexities and potential failures.
...
**Nota 1 — Herramientas no definidas en el código:**
`get_precise_location_info` y `get_general_area_info` son referenciadas en `tools=[...]`
pero nunca están definidas en el snippet. El código no puede ejecutarse sin estas funciones.
A diferencia de Cap.11 donde todos los imports eran reales, aquí hay referencias a funciones
que el lector debe implementar pero el capítulo no especifica qué deben retornar ni cómo
deben señalizar éxito/fallo.

**Nota 2 — El mecanismo de señalización de fallo está ausente:**
`fallback_handler` verifica `state["primary_location_failed"]` pero ningún código
en el snippet establece este valor. Preguntas sin respuesta:
- ¿Quién establece `state["primary_location_failed"] = True`?
- ¿Es `primary_handler` responsable de establecer su propio estado de fallo?
- ¿Lo hace el framework ADK automáticamente en caso de excepción del tool?
- ¿Si `get_precise_location_info` lanza una excepción, SequentialAgent continúa o detiene?
La lógica de fallback depende completamente de un mecanismo no mostrado.

**Nota 3 — `state["location_result"]` tampoco está establecida en el snippet:**
`response_agent` verifica `state["location_result"]` pero `primary_handler` y
`fallback_handler` no muestran cómo escriben en este estado. El flujo completo
del estado es implícito, no demostrado.

**Nota 4 — Duplicate AsciiDoc anchor `[[img-ref]]`:**
Ambas imágenes usan el mismo anchor `[[img-ref]]`. En AsciiDoc esto produce
comportamiento indefinido — el segundo anchor duplica el primero. Es un bug
de formato del documento fuente.

**Nota 5 — Referencias de mayor calidad que capítulos anteriores:**
Cap.11 tenía solo Wikipedia. Cap.12 tiene:
- McConnell (2004) Code Complete: referencia primaria de ingeniería de software, verificable
- arXiv:2412.00534 (Shi et al. 2024): preprint verificable sobre fault tolerance en MARL
- O'Neill (2022) Electronics 11(17) 2724: artículo de journal verificable
Las tres son fuentes primarias, a diferencia de Wikipedia.
Sin embargo, ninguna es citada en el cuerpo del texto — solo aparecen en References.
El cuerpo del texto no conecta ningún claim específico con ninguna referencia.

**Nota 6 — Trading bot sin requisitos regulatorios:**
"A trading bot attempting to execute a trade might encounter an 'insufficient funds' error
or a 'market closed' error. It needs to handle these exceptions by logging the error,
not repeatedly trying the same invalid trade."
Misma omisión detectada en Cap.10 y Cap.11: no menciona MiFID II, ACID, auditoría,
firma del usuario, ni compliance. El error handling descrito (log + no-retry) es
necesario pero insuficiente para un sistema financiero regulado.

**Nota 7 — Robotics sin estándares de seguridad funcional:**
"A robotic arm performing an assembly task might fail to pick up a component...
alert a human operator or switch to a different component."
No menciona ISO 10218 (robot safety), IEC 62061 (functional safety), ni SIL levels.
Tercer capítulo consecutivo con omisión sistemática de estándares en dominios regulados.

**Nota 8 — "transform AI agents from fragile...into robust, dependable components":**
Claim de transformación sin evidencia de que el patrón descrito (SequentialAgent con
state que no se muestra establecer) logra efectivamente "robust, dependable" behavior.
El código tiene un mecanismo de fallback con un gap de implementación crítico (Nota 2).
