```yml
created_at: 2026-04-19 11:08:07
project: THYROX
...
nota: |
  ...
  Capítulo más teórico de la serie: CoT, ToT, Self-Correction, PALMs, RLVR, ReAct,
  CoD, GoD, MASS, Deep Research, Scaling Inference Law.
  Un único bloque de código ADK runnable (PALMs) + un bloque LangGraph real
  (gemini-fullstack-langgraph-quickstart, Apache 2.0).
  6 referencias: 4 papers académicos (Wei 2022, Yao 2023 ×2, Gao 2023),
  1 paper arXiv Inference Scaling, 1 arXiv MASS. Ninguna citada inline.
  Mismo patrón CCV esperado (5ta confirmación).
  Fig numbering inconsistency: LangGraph code block labeled "Fig.4" pero
  LangGraph figure (DeepSearch) era Fig.6 en el texto.
```

# Input: Chapter 17 — Reasoning Techniques (texto completo, preservado verbatim)

---

## Introducción

Individual AI agents often face limitations when tackling complex, multifaceted problems, even with advanced capabilities.

This chapter delves into advanced reasoning methodologies for intelligent agents, focusing on multi-step logical inferences and problem-solving.

These techniques go beyond simple sequential operations, making the agent's internal reasoning explicit.
...
This means granting the agent, or the underlying LLM, more processing time or steps to process a query and generate a response.

Rather than a quick, single pass, the agent can engage in iterative refinement, explore multiple solution paths, or utilize external tools.

This extended processing time during inference often significantly enhances accuracy, coherence, and robustness, especially for complex problems requiring deeper analysis and deliberation.
...
Chain-of-Thought (CoT) prompting significantly enhances LLMs complex reasoning abilities by mimicking a step-by-step thought process (see Fig. 1).

Instead of providing a direct answer, CoT prompts guide the model to generate a sequence of intermediate reasoning steps.

This explicit breakdown allows LLMs to tackle complex problems by decomposing them into smaller, more manageable sub-problems.

This technique markedly improves the model's performance on tasks requiring multi-step reasoning, such as arithmetic, common sense reasoning, and symbolic manipulation.

A primary advantage of CoT is its ability to transform a difficult, single-step problem into a series of simpler steps, thereby increasing the transparency of the LLM's reasoning process.

This approach not only boosts accuracy but also offers valuable insights into the model's decision-making, aiding in debugging and comprehension.

CoT can be implemented using various strategies, including offering few-shot examples that demonstrate step-by-step reasoning or simply instructing the model to "think step by step." Its effectiveness stems from its ability to guide the model's internal processing toward a more deliberate and logical progression.

As a result, Chain-of-Thought has become a cornerstone technique for enabling advanced reasoning capabilities in contemporary LLMs.

This enhanced transparency and breakdown of complex problems into manageable sub-problems is particularly important for autonomous agents, as it enables them to perform more reliable and auditable actions in complex environments.

**[image: Fig.1 — CoT prompt alongside the detailed, step-by-step response generated by the agent]**

**CoT Example — Information Retrieval Agent:**
...
Tree-of-Thought (ToT) is a reasoning technique that builds upon Chain-of-Thought (CoT). It allows large language models to explore multiple reasoning paths by branching into different intermediate steps, forming a tree structure (see Fig. 2).

This approach supports complex problem-solving by enabling backtracking, self-correction, and exploration of alternative solutions.

Maintaining a tree of possibilities allows the model to evaluate various reasoning trajectories before finalizing an answer.

This iterative process enhances the model's ability to handle challenging tasks that require strategic planning and decision-making.

**[image: Fig.2 — Example of Tree of Thoughts]**

---

### Self-Correction

Self-correction, also known as self-refinement, is a crucial aspect of an agent's reasoning process, particularly within Chain-of-Thought prompting.

It involves the agent's internal evaluation of its generated content and intermediate thought processes.

This critical review enables the agent to identify ambiguities, information gaps, or inaccuracies in its understanding or solutions.

This iterative cycle of reviewing and refining allows the agent to adjust its approach, improve response quality, and ensure accuracy and thoroughness before delivering a final output.

This internal critique enhances the agent's capacity to produce reliable and high-quality results, as demonstrated in examples within the dedicated Chapter 4.

**Self-Correction Example — Social Media Post:**
...
Program-Aided Language Models (PALMs) integrate LLMs with symbolic reasoning capabilities.

This integration allows the LLM to generate and execute code, such as Python, as part of its problem-solving process.

PALMs offload complex calculations, logical operations, and data manipulation to a deterministic programming environment.

This approach utilizes the strengths of traditional programming for tasks where LLMs might exhibit limitations in accuracy or consistency.

When faced with symbolic challenges, the model can produce code, execute it, and convert the results into natural language.

This hybrid methodology combines the LLM's understanding and generation abilities with precise computation, enabling the model to address a wider range of complex problems with potentially increased reliability and accuracy.

This is important for agents as it allows them to perform more accurate and reliable actions by leveraging precise computation alongside their understanding and generation capabilities.
...
ReAct (Reasoning and Acting, see Fig. 3, where KB stands for Knowledge Base) is a paradigm that integrates Chain-of-Thought (CoT) prompting with an agent's ability to interact with external environments through tools.

Unlike generative models that produce a final answer, a ReAct agent reasons about which actions to take.

This reasoning phase involves an internal planning process, similar to CoT, where the agent determines its next steps, considers available tools, and anticipates outcomes.
...
**[image: Fig.3 — Reasoning and Act]**

ReAct operates in an interleaved manner: the agent executes an action, observes the outcome, and incorporates this observation into subsequent reasoning.
...
This provides a more robust and flexible problem-solving approach compared to linear CoT, as the agent responds to real-time feedback.

By combining language model understanding and generation with the capability to use tools, ReAct enables agents to perform complex tasks requiring both reasoning and practical execution.

This approach is crucial for agents as it allows them to not only reason but also to practically execute steps and interact with dynamic environments.
...
**[image: Fig.5 — Google Deep Research for Information Gathering]**
...
During this time, the AI works on your behalf in an agentic way. It autonomously performs a series of sophisticated steps:
...
The Inference Scaling Law differs from the more familiar scaling laws for training, which focus on how model quality improves with increased data volume and computational power during a model's creation.

Instead, this law specifically examines the dynamic trade-offs that occur when an LLM is actively generating an output or answer.
...
This doesn't necessarily mean using a more powerful GPU, but rather employing more sophisticated or resource-intensive inference strategies.

A prime example of such a strategy is instructing the model to generate multiple potential answers—perhaps through techniques like diverse beam search or self-consistency methods—and then employing a selection mechanism to identify the most optimal output.

This iterative refinement or multiple-candidate generation process demands more computational cycles but can significantly elevate the quality of the final response.

This principle offers a crucial framework for informed and economically sound decision-making in the deployment of Agents systems.

It challenges the intuitive notion that a larger model will always yield better performance.
...
The "thinking budget" here refers to the additional computational steps or complex algorithms applied during inference, allowing the smaller model to explore a wider range of possibilities or apply more rigorous internal checks before settling on an answer.

Consequently, the Scaling Inference Law becomes fundamental to constructing efficient and cost-effective Agentic systems.

It provides a methodology for meticulously balancing several interconnected factors:
...
The DeepSearch code, open-sourced by Google, is available through the `gemini-fullstack-langgraph-quickstart` repository (Fig. 6).

This repository provides a template for developers to construct full-stack AI agents using Gemini 2.5 and the LangGraph orchestration framework.

This open-source stack facilitates experimentation with agent-based architectures and can be integrated with local LLMs such as Gemma.

It utilizes Docker and modular project scaffolding for rapid prototyping.

It should be noted that this release serves as a well-structured demonstration and is not intended as a production-ready backend.

**[image: Fig.6 — Example of DeepSearch with multiple Reflection steps]**
...
**Requirements:** Node.js, npm, Python 3.8+, Google Gemini API key.

**Production deployment:** Redis for streaming real-time output, Postgres database for managing data. Docker image via `docker-compose up`.

**Stack:** React with Vite, Tailwind CSS, Shadcn UI, LangGraph, Google Gemini. Apache License 2.0.
...
*Fig.4: Example of DeepSearch with LangGraph (code from `backend/src/agent/graph.py`)* [nota: el capítulo lo etiqueta Fig.4 pero en el texto era Fig.6 para la imagen de DeepSearch]
...
**[image: Fig.7 — Reasoning design pattern]**
...
**Nota 1 — Capítulo más teórico de la serie: 9 técnicas sin implementación propia:**
Cap.17 describe CoT, ToT, Self-Correction, PALMs, RLVR, ReAct, CoD, GoD, MASS, Deep Research
y Scaling Inference Law. La mayoría son frameworks bien documentados académicamente (Wei 2022,
Yao 2023). El único código "propio" del capítulo es el snippet de PALMs en ADK (3 agentes:
SearchAgent, CodeAgent, RootAgent) — un bloque pequeño y el LangGraph snippet de DeepSearch.

**Nota 2 — PALMs code: `code_executor=[BuiltInCodeExecutor]` — posible bug de tipo:**
```python
coding_agent = Agent(
    ...
    code_executor=[BuiltInCodeExecutor],  # lista con la clase, no instancia
)
```
El parámetro `code_executor` recibe `[BuiltInCodeExecutor]` — una lista que contiene
la clase, no una instancia. En ADK, `code_executor` puede esperarse como instancia directa
(`BuiltInCodeExecutor()`) o como clase. Si ADK espera una instancia, pasar la clase sin
`()` causará error en runtime. Requiere verificación contra la API de ADK.

**Nota 3 — CoD atribuido a "Microsoft" sin referencia:**
"CoD (Chain of Debates) is a formal AI framework proposed by Microsoft..."
Microsoft está nombrado como autor pero no hay referencia #N en la lista de referencias
para este claim específico. Las referencias 1-6 cubren CoT (Wei), ToT (Yao), PALMs (Gao),
ReAct (Yao), Inference Scaling (anon 2024), MASS (arXiv:2502.02533). CoD y GoD no tienen
referencia citada — ni inline ni en bibliography.

**Nota 4 — Fig numbering inconsistency (Fig.4 usado dos veces):**
El texto del capítulo asigna Fig.4 a la imagen del framework MASS. Luego, el código
LangGraph tiene el caption "Fig.4: Example of DeepSearch with LangGraph" — mismo número.
Fig.6 en el texto corresponde a la imagen de DeepSearch ("Example of DeepSearch with
multiple Reflection steps"). El código debería ser Fig.6 o no tener número de figura.

**Nota 5 — Self-Correction referencia "Chapter 4" sin explicitar el contenido:**
"as demonstrated in examples within the dedicated Chapter 4"
Cross-referencia interna que no puede verificarse sin acceder a Cap.4. El claim de calidad
("more refined, precise, and superior results") es performativo — depende de Cap.4.

**Nota 6 — GoD sin referencia académica ni implementación:**
Graph of Debates es descrito como "an advanced Agentic framework" sin:
- Referencia bibliográfica
- Paper académico
- Repositorio de código
- Comparación con CoD en términos de performance

Si GoD es una contribución original del libro, es un claim de diseño sin validación externa.
Si es un framework existente, falta la referencia.

**Nota 7 — RLVR: descripción cualitativa sin métricas:**
"By training the model on problems with known correct answers... it learns through trial
and error to generate effective, long-form reasoning."
El claim central de RLVR (que produce mejores resultados que CoT estándar) no tiene
referencia específica. Se menciona que los modelos son "reasoning models" sin nombrar
ejemplos concretos (o1, Deepseek-R1, etc.) ni citar papers de RLVR.

**Nota 8 — Scaling Inference Law: "law" sin fórmula matemática:**
El capítulo llama a este principio una "law" pero no presenta ninguna fórmula matemática.
La referencia #5 ("Inference Scaling Laws: An Empirical Analysis...") existe pero no está
citada inline al describir la ley. Sin fórmula ni cita inline, el término "law" es
performativo — más fuerte que "principle" o "observation" sin evidencia cuantitativa visible.

**Nota 9 — CCV esperado confirmado: 6 referencias, ninguna inline:**
Misma estructura que Cap.12, Cap.13, Cap.14, Cap.15. Referencias en bibliography final,
cero inline. La hipótesis CCV predice que estos claims tienen calibración base sin
elevación por referencia. A diferencia de capítulos anteriores, Cap.17 tiene 4 papers
académicos peer-reviewed reales (Wei, Yao ×2, Gao) — pero ninguno citado inline.

**Nota 10 — "Named Mechanism vs. Implementation": Cap.17 rompe el patrón parcialmente:**
A diferencia de Cap.10-16, Cap.17 no promete implementar un mecanismo específico en código.
El capítulo es explícitamente un survey de técnicas de reasoning. El código de PALMs es
un ejemplo ilustrativo, no una implementación del mecanismo del título ("Reasoning Techniques").
El patrón se rompe porque el capítulo no hace el claim de implementar lo que describe —
describe frameworks existentes y los vincula con el código de terceros.

**Nota 11 — `code_executor` vs `code_executors` en ADK:**
```python
coding_agent = Agent(
    ...
    code_executor=[BuiltInCodeExecutor],
...
```
El nombre del parámetro en el snippet es `code_executor` (singular). El import en
`discover/rag-pattern-input.md` y documentación ADK pueden usar `code_executors`
(plural). Inconsistencia de naming a verificar contra la API ADK actual.

**Nota 12 — LangGraph snippet: imports no mostrados (igual que Cap.15):**
`StateGraph`, `OverallState`, `Configuration`, `generate_query`, `web_research`,
`reflection`, `finalize_answer`, `continue_to_web_research`, `evaluate_research`,
`START`, `END` — ninguno importado en el snippet. El capítulo no hace la aclaración
explícita que hizo Cap.15 ("only the code required to explain this functionality is shown").
Sin esa aclaración, el snippet parece código incompleto no intencional.
