...
nota: |
  ...
  A diferencia de capítulos anteriores: el código proviene de repositorios
  reales de GitHub (google-a2a/a2a-samples). Las URLs son verificables.
  7 referencias, todas URLs (ninguna arXiv, ninguna journal peer-reviewed).
  Inconsistencia detectada: Key Takeaways menciona "tasks/send" y
  "tasks/sendSubscribe" pero los JSON-RPC payloads usan "sendTask" y
  "sendTaskSubscribe" — naming inconsistency en el mismo capítulo.
  datetime.datetime.now() evaluado en tiempo de creación del agente,
  no en cada invocación — potencial bug de contexto temporal estático.
```

# Input: Chapter 15 — Inter-Agent Communication (A2A) (texto completo, preservado verbatim)
...
To overcome this, Inter-Agent Communication (A2A) enables diverse AI agents, potentially built with different frameworks, to collaborate effectively.

This collaboration involves seamless coordination, task delegation, and information exchange.

Google's A2A protocol is an open standard designed to facilitate this universal communication.
...
It ensures interoperability, allowing AI agents developed with technologies like LangGraph, CrewAI, or Google ADK to work together regardless of their origin or framework differences.
...
Microsoft plans to integrate A2A into Azure AI Foundry and Copilot Studio, demonstrating its commitment to open protocols.

Additionally, Auth0 and SAP are integrating A2A support into their platforms and agents.
...
The A2A protocol provides a structured approach for agent interactions, built upon several core concepts.

A thorough grasp of these concepts is crucial for anyone developing or integrating with A2A-compliant systems.

The foundational pillars of A2A include Core Actors, Agent Card, Agent Discovery, Communication and Tasks, Interaction mechanisms, and Security, all of which will be reviewed in detail.
...
This file contains key information for client interaction and automatic discovery, including the agent's identity, endpoint URL, and version.

It also details supported capabilities like streaming or push notifications, specific skills, default input/output modes, and authentication requirements.

Below is an example of an Agent Card for a WeatherBot.
...
It allows clients to find Agent Cards, which describe the capabilities of available A2A Servers.

Several strategies exist for this process:
...
A2A provides multiple interaction methods to suit a variety of AI application needs, each with a distinct mechanism:
...
The Agent Card specifies whether an agent supports streaming or push notification capabilities. Furthermore, A2A is modality-agnostic, meaning it can facilitate these interaction patterns not just for text, but also for other data types like audio and video, enabling rich, multimodal AI applications.

Both streaming and push notification capabilities are specified within the Agent Card.
...
Inter-Agent Communication (A2A) is a vital component of system architecture, enabling secure and seamless data exchange among agents.

It ensures robustness and integrity through several built-in mechanisms.
...
A2A is a protocol that complements Anthropic's Model Context Protocol (MCP) (see Fig. 1).
...
**[image: Fig.1 — Comparison A2A and MCP Protocols]**

The goal of A2A is to enhance efficiency, reduce integration costs, and foster innovation and interoperability in the development of complex, multi-agent AI systems.
...
Let's examine the practical applications of the A2A protocol.

The repository at https://github.com/google-a2a/a2a-samples/tree/main/samples provides examples in Java, Go, and Python that illustrate how various agent frameworks, such as LangGraph, CrewAI, Azure AI Foundry, and AG2, can communicate using A2A. All code in this repository is released under the Apache 2.0 license.

To further illustrate A2A's core concepts, we will review code excerpts focusing on setting up an A2A Server using an ADK-based agent with Google-authenticated tools.

Looking at https://github.com/google-a2a/a2a-samples/blob/main/samples/python/agents/birthday_planner_adk/calendar_agent/adk_agent.py
...
**[image: Fig.2 — A2A inter-agent communication pattern]**
...
**Nota 1 — Inconsistencia de naming en métodos JSON-RPC:**
Los JSON-RPC payloads del capítulo usan `"method": "sendTask"` y `"method": "sendTaskSubscribe"`.
Los Key Takeaways del mismo capítulo mencionan `tasks/send` y `tasks/sendSubscribe`.
Son convenciones de naming distintas para el mismo concepto dentro del mismo capítulo.
La especificación A2A oficial en https://a2a-protocol.org/latest/ debe consultarse
para determinar cuál es el nombre canónico del método. Este es un error de consistencia
interna — no afecta la arquitectura pero puede confundir al implementador.

**Nota 2 — `datetime.datetime.now()` evaluado en tiempo de creación del agente:**
```python
instruction=f"""
...
Today is {datetime.datetime.now()}.
""",
```
La f-string se evalúa **una sola vez** cuando se ejecuta `create_agent()`. Si el agente
es creado al inicio del servidor y corre por días, la fecha en `instruction` queda estática.
El capítulo describe esto como "dynamically incorporate the current date for temporal context"
— pero la incorporación ocurre en tiempo de creación, no en tiempo de invocación. Para que
sea verdaderamente dinámico, la fecha debería calcularse dentro de un callback o en el prompt
de cada invocación.

**Nota 3 — Las URLs del código son verificables (diferencia con capítulos anteriores):**
A diferencia de Cap.12/13 donde las funciones no estaban definidas, Cap.15 apunta a
repositorios reales: https://github.com/google-a2a/a2a-samples
El código es accesible, open-source (Apache 2.0), y puede ejecutarse con las credenciales
correctas. Esta es la mayor diferencia respecto a capítulos anteriores: el código de ejemplo
no es un snippet aislado sino una referencia a implementación real verificable.

**Nota 4 — `InMemoryArtifactService`, `InMemorySessionService`, `InMemoryMemoryService`:**
Los tres servicios son in-memory — efímeros, sin persistencia. Al igual que
`EmbeddedOptions()` en Cap.14 (Weaviate in-memory), son apropiados para desarrollo
pero no para producción. El capítulo no hace esta distinción explícita.

**Nota 5 — Los imports del snippet `__main__.py` no están mostrados:**
`AgentSkill`, `AgentCard`, `AgentCapabilities`, `ADKAgentExecutor`, `Runner`,
...
`Route`, `Starlette`, `Request`, `PlainTextResponse`, `uvicorn` — ninguno de estos
imports está en el snippet. El capítulo indica "only the code required to explain this
functionality is shown" — aclaración honesta del scope del snippet, a diferencia de
capítulos anteriores que omitían imports sin señalarlo.

**Nota 6 — Los claims sobre adopción de la industria son verificables parcialmente:**
"A2A is supported by... Atlassian, Box, LangChain, MongoDB, Salesforce, SAP, ServiceNow"
"Microsoft plans to integrate A2A into Azure AI Foundry"
Estos son claims sobre el estado del ecosistema en la fecha de escritura del capítulo.
Son verificables consultando los anuncios oficiales de cada empresa — más verificables
que los claims cualitativos de capítulos anteriores, pero pueden ser obsoletos rápidamente
dado que A2A es una especificación reciente (2025).

**Nota 7 — 7 referencias, ninguna arXiv ni journal peer-reviewed:**
Todas las referencias son URLs: blogs (Trickle.so), repositorios GitHub, documentación
oficial Google, O'Reilly Radar, codelabs. No hay papers académicos.
Sin embargo, el tipo de evidencia es apropiado para un protocolo técnico nuevo (2025)
que aún no tiene literatura académica extensa. La referencia de O'Reilly Radar es
una fuente de mayor autoridad que un blog, aunque no peer-reviewed.
El patrón CCV (citas no inline) se mantiene: ninguna referencia está citada inline.

**Nota 8 — La URL en Ref.6 es idéntica a Ref.1:**
Ref.1: https://www.trickle.so/blog/how-to-build-google-a2a-project
Ref.6: https://www.trickle.so/blog/how-to-build-google-a2a-project
Misma URL duplicada en la lista de referencias. Bug editorial del documento fuente.
